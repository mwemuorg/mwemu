...
// PCMPESTRI xmm1, xmm2/m128, imm8 : SSE4.2 packed compare of explicit-length (EAX/EDX) strings, returning an index in ECX and setting flags.
pub fn execute(emu: &mut Emu, ins: &Instruction, _instruction_sz: usize, _rep_step: bool) -> bool {
    ...
    // Explicit lengths from EAX (string1) and EDX (string2), abs-valued and
    // saturated to the number of elements.
    let _ = is_null;
    let len1 = ((emu.regs().get_eax() as i32) as i64)
        .unsigned_abs()
        .min(n as u64) as usize;
    let len2 = ((emu.regs().get_edx() as i32) as i64)
        .unsigned_abs()
        .min(n as u64) as usize;
